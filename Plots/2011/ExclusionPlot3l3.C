...
//and now  the exclusion limits
  ...
  //  TH2D* hist = new TH2D("h","h",100,0,300,100,120,450);
  TH2D* hist = new TH2D("h","h",100,50,300,100,120,450);
  hist->Draw();  
  ...
  TLegend* myleg = new TLegend(0.25,0.7,0.5,0.9,"95% C.L. CLs Limits","brNDC");;
  ...
  expectedP13l->Draw("samelp");
  expectedM13l->Draw("samelp");
  expected3l->Draw("samelp");
  observed3l->Draw("samelp");
  ...
  TLatex* sugraLabel = new TLatex(180,180.,"tan#beta = 3, A_{0} = 0, #mu > 0");
  ...
  TLatex* lumilabel = new TLatex(170,460,"#sqrt{s} = 7 TeV,   L_{int} = 2.1 fb^{-1}");
  ...
  TLatex* cmslabel = new TLatex(60.,460,"CMS Preliminary");
  ...
  */
  ...
  if( plotLO_ ){
    ...
    //    cvsSys->SaveAs("ExclusionLimit_tanb3.eps");
    cvsSys->SaveAs("ExclusionLimit_tanb3.pdf");
    // cvsSys->SaveAs("ExclusionLimit_3.png");
  }
