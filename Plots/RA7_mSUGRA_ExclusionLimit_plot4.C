...
//   First->GetYaxis()->SetTitleOffset(0.8);
 ...
 Double_t *dum = 0;
   ...
   LO_obs->SetLineStyle(0);
   LO_obs->SetLineWidth(5);
  ...
  //first one
   ...
   grshade2->SetLineColor(kBlue+2);
   grshade2->SetLineWidth(3);  
   grshade2->SetLineStyle(3);  
 ...
 for(vector<TH1F*>::iterator at = exclusionPlots.begin();at != exclusionPlots.end();++at){
  ...
  TLegend* myleg = new TLegend(0.25,0.75,0.55,0.90,NULL,"brNDC");
  ...
  myleg->AddEntry(LO_obs,"CMS LO Observed 95%CL Limit","L"); 
  myleg->AddEntry(NLO_obs,"CMS NLO Observed 95%CL Limit","L"); 
  myleg->AddEntry(NLO_exp,"CMS NLO Expected 95%CL Limit","L");    
  myleg->AddEntry(grshade2,"Expected Limit #pm 1#sigma","l");
  ...
  for (int it=1;it<4;it++) {   
    lngl[it]->Draw("same");   
    lnsq[it]->Draw("same");
    sq_text[it]->Draw();    gl_text[it]->Draw();
  }
...
//     tLM1->Draw("same");
  ...
  cvsSys->SaveAs("RA7_mSUGRA_ExclusionLimit_plot4.pdf");
//   cvsSys->SaveAs("RA1_ExclusionLimit_tanb"+tanb+".png");
//   cvsSys->SaveAs("RA1_ExclusionLimit_tanb"+tanb+".eps");
//   cvsSys->SaveAs("MSUGRA_RA1Style_v6.eps");
...
TLegend* makeExpLegend(TGraph& sg_gr, TGraph& sgd_gr,TGraph& ch_gr,TGraph& sl_gr,TGraph& tev_sn,Double_t txtsz,Int_t tanbeta){
  TLegend* legexp = new TLegend(0.63,0.65,0.91,0.9,NULL,"brNDC");
