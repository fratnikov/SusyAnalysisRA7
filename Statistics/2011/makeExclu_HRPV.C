...
double distance2 (vector<double>& xx, vector<double>& yy, unsigned i, unsigned j) {
  return (xx[i]-xx[j])*(xx[i]-xx[j])+10*(yy[i]-yy[j])*(yy[i]-yy[j]);
}

void orderInDistance (vector<double>& xx, vector<double>& yy) {
  orderInY (xx,yy);
  cout << "trace 0" << endl;
  // the most top point
  for (unsigned i = 1; i < yy.size(); ++i) {
    if (yy[i] > yy[0]) {
      swap (yy[i], yy[0]);
      swap (xx[i], xx[0]);
    }
  ...
  for (unsigned i = 1; i < xx.size(); ++i) {
    double minR2 = distance2 (xx, yy, i, i-1);
    for (unsigned j = i+1; j < xx.size(); ++j) {
      if (distance2 (xx, yy, j, i-1)< minR2) {
	cout << "swapping " << i << ':' << j << " " << xx[i]<<':'<<yy[i]<<"<->"<<xx[j]<<':'<<yy[j]<<" " <<minR2<<"->"<<distance2 (xx, yy, j, i-1)<<endl;
	minR2 = distance2 (xx, yy, j, i-1);
	swap (yy[j], yy[i]);
	swap (xx[j], xx[i]);
      }
    }
    cout << "trace 3 ->" << i <<' '<<xx[i]<<':'<<yy[i]<<endl;
  }
  cout << "trace 99" << endl;
}
...
//   	       << endl;
  ...
  int binY2 = hist->GetYaxis()->FindBin (800.);
  for (int iy = binY2+1; iy >= binY2-1; --iy) {
    double y = hist->GetYaxis()->GetBinCenter (iy);
    // cout << "y bin: " << iy << ' ' << y << endl;
    double logyprev = 2*log (hist->GetBinContent(nBinsX, iy)) - log(hist->GetBinContent(nBinsX-1,iy));
    int ix = nBinsX;
...
void drawAll() {
  ...
  TH2D* frame = new TH2D ("LRPV", "LRPV", 1, 900, 2200, 1, 600, 2000);
  frame->GetXaxis()->SetTitle ("m_1");
  frame->GetYaxis()->SetTitle ("m_2");
  frame->Draw();
  TGraph* observed = makeCurve (hObsLimitSmooth, 1., 1400, 1000);
  observed->SetLineColor(kBlack);
   TGraph* expected = makeCurve (hExpLimitSmooth, 1., 1400, 1000);
   expected->SetLineColor(kBlue);
   TGraph* expectedP1 = makeCurve (hExpP1LimitSmooth, 1., 1400, 1000);
   expectedP1->SetLineColor(kMagenta);
   TGraph* expectedM1 = makeCurve (hExpM1LimitSmooth, 1., 1400, 1000);
   expectedM1->SetLineColor(kMagenta);
  
   TGraph* expectedP2 = makeCurve (hExpP2LimitSmooth, 1., 1400, 1000);
   expectedP2->SetLineColor(kGreen);
   TGraph* expectedM2 = makeCurve (hExpM2LimitSmooth, 1., 1400, 1000);
