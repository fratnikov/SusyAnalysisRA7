...
void drawAll() {
  ...
  TH2D* frame = new TH2D ("LRPV", "LRPV", 1, 900, 2000, 1, 600, 2000);
  frame->GetXaxis()->SetTitle ("m_1");
  frame->GetYaxis()->SetTitle ("m_2");
  frame->Draw();
  TGraph* observed = makeCurve (hObsLimitSmooth, 1., 1400, 1300);
  observed->SetLineColor(kBlack);
  TGraph* expected = makeCurve (hExpLimitSmooth, 1., 1500, 1200);
  expected->SetLineColor(kBlue);
  TGraph* expectedP1 = makeCurve (hExpP1LimitSmooth, 1., 1400, 1300);
  expectedP1->SetLineColor(kMagenta);
  TGraph* expectedM1 = makeCurve (hExpM1LimitSmooth, 1., 1400, 1300);
  expectedM1->SetLineColor(kMagenta);
  
  TGraph* expectedP2 = makeCurve (hExpP2LimitSmooth, 1., 1400, 1300);
  expectedP2->SetLineColor(kGreen);
  TGraph* expectedM2 = makeCurve (hExpM2LimitSmooth, 1., 1200, 1400);
