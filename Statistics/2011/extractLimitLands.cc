...
void run(TString inFileName, int m0, int m12){
  string plotName ("blah");
	...
	cout<<"EXPECTED LIMIT BANDS from(obs, -1s,median,1s): ";
	cout<<dat<<"+/-"<<dat_err<<", ";
	cout<<m2s<<"+/-"<<m2s_err<<", ";
	cout<<m1s<<"+/-"<<m1s_err<<", ";
	cout<<med<<"+/-"<<med_err<<", ";
	cout<<p1s<<"+/-"<<p1s_err<<", ";
	cout<<p2s<<"+/-"<<p2s_err<<endl;

	cout<<"Observed data limit: "<<dat<<endl;
	cout<<"Observed data limit: "<<dat<<" +/- "<<dat_err<<endl;
	cout<<"expected median limit: "<<med<<" +/- "<<med_err<<endl;

	ofstream output ("combinedModel.out", ios_base::app);
	output << m0<<':'<<m12
	       << " cls: " << dat << " " << med << " ( " << m1s << " : " << p1s << " ) " << " ( " << m2s << " : " << p2s << " ) " << endl;  
	output.close();
	cout << m0<<':'<<m12
	       << " cls: " << dat << " " << med << " ( " << m1s << " : " << p1s << " ) " << " ( " << m2s << " : " << p2s << " ) " << endl;
...
int main (int argc, char* argv[]) {
  if (argc < 4) {
    cout << argv[0] << " <Lands ROOT file> m0 m12 " << endl;
    return 0;
  }
